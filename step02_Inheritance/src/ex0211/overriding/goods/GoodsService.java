...
public class GoodsService{
	...
	public GoodsService(String [][] data) {
		...
		System.out.println("goodsArr.length = " + goodsArr.length);

	}
	
   /**
      Goods를 생성해서 값을 설정하고 생성된 Goods를 리턴하는 생성자
   */
   ...
   */
   public InsertResult insert(Goods goods){
	   ...
	   return InsertResult.INSERT_SUCESS;
   }
   ...
   */
   public boolean update(Goods goods){ //수정하려는 코드, 변경값 - 가격, 설명
		/*
		 * for(int i=0; i<count; i++) {
		 * if(goodsArr[i].getCode().equals(goods.getCode())) { goodsArr[i] = new Goods(
