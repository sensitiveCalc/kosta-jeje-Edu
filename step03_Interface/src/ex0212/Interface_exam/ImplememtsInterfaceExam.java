...
interface InterfaceExam01 extends InterfaceExam00{
	...
	//protected int k = 999;
	public int k = 999;
	void interfaceExam01(int i, int k); //메소드는 무조건 추상화로(public abstract 자동 생략)
...
interface InterfaceExam02{
	int j =747;
	//private int privateInt = 8; // private는 body를 달거나 아니면 public을 붙이거나 제외한다
	int privateInt = 8;
...
//implements를 할 경우 부모 인터페이스의 메소드들을 모두 선언해야 함
...
class SubClassExam04 extends SubClassExam02{
	...
	void printSuperString(){
		InterfaceExam01 ie01; /*= new InterfaceExam01(); */ //인터페이스는 타입은 가능하지만 생성은 불가
...
public class ImplememtsInterfaceExam{ 
	public static void main(String args[]){
		SubClassExam02 sub02sub04 = new SubClassExam04("SubClassExam02", "sub02sub04"); //인터페이스를 다중상속하여 생성
